    ...
    for check in checks:
        ...
        try:
            if check(host, port, timeout):
                ...
                return {
                    "protocol": "socks5" if check is check_socks5 else "http",
                    "healthy": True,
                    "latency_ms": ms,
                }
        except OSError:
...
def api_post(
    ...
    headers = {
        ...
        "user-agent": f"hyu-lan-agent/{VERSION}",
    }
    if authenticated:
...
def pair(config: dict[str, Any], pairing_key: str, name: str, force: bool = False) -> None:
    if config.get("agent_id") and config.get("agent_token") and not force:
        raise RuntimeError(
            "This device is already paired. Remove the old agent from the dashboard "
            "and run pair again with --force if you intentionally want to replace it."
        )
...
def find_proxy(
    proxies: list[dict[str, Any]], host: str, port: int, protocol: str
) -> dict[str, Any] | None:
    ...
    return next(
        (
            ...
            if endpoint_key(proxy["host"], proxy["port"], proxy["protocol"]) == wanted
        ),
        None,
    )
...
def execute_command(
    ...
    try:
        if kind == "rescan":
            ...
            proto = str(payload["protocol"]).lower()
            ...
            try:
                healthy = (
                    check_socks5(host, port, float(config.get("scan_timeout_seconds", 0.3)))
                    if proto == "socks5"
                    else check_http_proxy(host, port, float(config.get("scan_timeout_seconds", 0.3)))
                )
            except OSError:
                healthy = False
            ...
            port = int(payload["port"])
            proto = str(payload["protocol"]).lower()
            if not usable_private_ipv4(host) or proto not in {"http", "socks5"}:
                raise ValueError("Invalid proxy endpoint")
...
def run(config: dict[str, Any], no_scan: bool = False) -> None:
    if not config.get("agent_id") or not config.get("agent_token"):
        raise RuntimeError(
            "Agent is not paired. Run: python agent/agent.py pair --key <KEY> --name <NAME>"
        )
    ...
    if no_scan:
        next_scan_at = (
            time.monotonic() + rescan_seconds if rescan_seconds else float("inf")
        )
        ...
        try:
            ...
            print(
                f"Initial scan failed; heartbeat will continue and existing registry "
                f"entries will be kept as stale/unhealthy: {exc}",
                file=sys.stderr,
            )
        next_scan_at = (
            time.monotonic() + rescan_seconds if rescan_seconds else float("inf")
    ...
    while True:
        ...
        for proxy in proxies:
            item = dict(proxy)
            item["enabled"] = (
                endpoint_key(item["host"], item["port"], item["protocol"]) not in disabled
            )
        ...
        try:
            ...
            for command in commands:
                ...
                try:
                    ...
                    print(
                        f"Could not report command result; the command may be retried: "
                        f"{report_error}",
                        file=sys.stderr,
                    )

            print(
                f"heartbeat ok · proxies={len(proxies)} · "
                f"roundtrip={last_roundtrip_ms}ms"
            )
            ...
            if exc.status == 401:
                raise RuntimeError(
                    "Agent credential was rejected. Remove this device from the "
                    "dashboard, then pair it again."
                ) from exc
            sleep_for = min(60, interval * (2 ** min(consecutive_failures, 3)))
            print(
                f"heartbeat failed: {exc} · retry in {sleep_for}s",
                file=sys.stderr,
            )
...
def reset_local_config(config: dict[str, Any], confirmed: bool) -> None:
    if not confirmed:
        raise RuntimeError(
            "Refusing to remove the local credential without --yes. "
            "Remove the agent from the dashboard first."
        )
    if CONFIG_PATH.exists():
        CONFIG_PATH.unlink()
    print(
        "Local pairing credential removed. The server-side agent entry is not "
        "deleted automatically; remove it from the dashboard if it still exists."
    )
...
def main() -> None:
    ...
    pair_p = sub.add_parser(
        "pair", help="Pair this device using a one-time dashboard key"
    )
    ...
    pair_p.add_argument(
        "--force",
        action="store_true",
        help="Replace an existing local pairing credential",
    )
    ...
    run_p.add_argument(
        "--no-scan",
        action="store_true",
        help="Start heartbeat without an initial LAN scan",
    )
    ...
    reset_p = sub.add_parser(
        "reset", help="Remove the local pairing credential/configuration"
    )
    ...
    if args.command == "pair":
        ...
        print(
            ...
            if args.json
                ...
                f"{p['host']}:{p['port']} {p['protocol'].upper()} "
                f"{p['latency_ms']}ms"
                for p in result
            )
        )
