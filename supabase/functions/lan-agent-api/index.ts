...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (req.method === "POST" && route === "pair") {
      ...
      const name =
        String(body.name || "LAN Agent").trim().slice(0, 80) || "LAN Agent";
      ...
      if (error) {
        return json(401, {
          ...
          detail: "The pairing key is invalid, expired, or already used.",
        });
      }
    ...
    if (req.method === "POST" && route === "heartbeat") {
      ...
      const rawTailscaleIp = body.tailscale_ip
        ? String(body.tailscale_ip).trim()
        : "";
      const tailscaleIp = isTailscaleIpv4(rawTailscaleIp)
        ? rawTailscaleIp
        : null;
      ...
      if (body.agent_version) {
        update.agent_version = String(body.agent_version).trim().slice(0, 40);
      }
      ...
      const normalized = proxies
        .map((proxy: any) => {
          ...
          return {
            ...
            last_checked_at: now,
            metadata:
              typeof proxy.metadata === "object" && proxy.metadata
                ? proxy.metadata
                : {},
            updated_at: now,
          };
      ...
      if (normalized.length) {
        const { error: upsertError } = await admin
          .from("lan_proxies")
          .upsert(normalized, {
            onConflict: "agent_id,host,port,protocol",
          });
        if (upsertError) {
          return json(500, {
            error: "proxy_upsert_failed",
      ...
      if (snapshotComplete) {
        ...
        const staleIds = (existing || [])
          .filter(
            (proxy: any) =>
              !keep.has(`${proxy.host}|${proxy.port}|${proxy.protocol}`),
          )
    ...
    if (req.method === "POST" && route === "command-result") {
      ...
      if (!commandId) {
        return json(400, { error: "missing_command_id" });
      }
      ...
      const { error: updateError } = await admin
        ...
        .update({
          ...
          error_message: ok
            ? null
            : String(body.error || "agent command failed").slice(0, 500),
        })
        .eq("id", commandId)
        .eq("agent_id", auth.agentId);
